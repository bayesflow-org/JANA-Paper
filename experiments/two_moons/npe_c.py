...
def two_moons_npe_c(simulation_budget,
                    seed,
                    prior,
                    simulator,
    ...
    density_estimator = inference.append_simulations(theta, x).train(max_num_epochs=50,
    ...
    np.savetxt(f'output/{dir_prefix}npec_post_sims{simulation_budget}_seed{seed}.csv',
...
if __name__ == "__main__":
    ...
    for seed in config.seeds:
        for simulation_budget in [2000, 6000, 10000]:
            two_moons_npe_c(simulation_budget=simulation_budget,
                            seed=seed,
