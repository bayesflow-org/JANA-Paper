...
def two_moons_jana(simulation_budget,
                   seed,
                   generator,
                   batch_size=32,
    ...
    # Create amortizers
    inference_net = InvertibleNetwork(num_params=2, num_coupling_layers=4,
                                      coupling_design='spline', permutation='learnable')
    likelihood_net = InvertibleNetwork(num_params=2, num_coupling_layers=5,
                                       coupling_design='interleaved', permutation='learnable')

    amortizer = AmortizedPosteriorLikelihood(AmortizedPosterior(inference_net),
                                             AmortizedLikelihood(likelihood_net))
    ...
    start = time.time()
    _ = trainer.train_offline(sims, epochs=64, batch_size=batch_size, validation_sims=300)
    end = time.time()
    run_time = end - start
    ...
    np.savetxt(f'output/{dir_prefix}jana_post_sims{simulation_budget}_seed{seed}.csv',
               post_samples,
               delimiter=',')
    ...
    start = time.time()
    post_samples = amortizer.sample_parameters(obs_data, num_posterior_samples)
    post_pred_samples = amortizer.sample_data({'conditions': post_samples}, n_samples=1).squeeze()
    end = time.time()
    run_time_posterior_predictive_inference = (end - start)

    np.savetxt(f'output/{dir_prefix}jana_post_pred_sims{simulation_budget}_seed{seed}.csv',
               post_pred_samples,
               delimiter=',')
    ...
    if simulation_budget == 10000:
        with open(f'output/{dir_prefix}jana_metadata_seed{seed}.txt', 'w') as f:
            ...
            f.write(f'Inference time (posterior) 10k: {run_time_posterior_inference:.4f}\n')
            f.write(f'Inference time (posterior predictive) 10k: {run_time_posterior_predictive_inference:.4f}\n')
...
if __name__ == "__main__":
    ...
    for seed in config.seeds:
        for batch_size, simulation_budget in zip(batch_sizes, simulation_budgets):
            ...
            simulator_1 = Simulator(batch_simulator_fun=two_moons_wiqvist_numpy.simulator_numpy_batched)
            generator_1 = GenerativeModel(prior=prior_1, simulator=simulator_1)

            two_moons_jana(simulation_budget=simulation_budget,
                           seed=seed,
            ...
            generator_2 = benchmarks.Benchmark("two_moons",
                                               prior_kwargs={'lower_bound': -2,
                                                             'upper_bound': 2}).generative_model
