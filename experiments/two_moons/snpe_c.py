...
def two_moons_snpe_c(num_simulations_per_round,
                     num_rounds,
                     seed,
                     prior,
                     simulator,
    ...
    for i in range(num_rounds):
        ...
        density_estimator = inference.append_simulations(theta, x, proposal=proposal).train(max_num_epochs=50,
                                                                                            learning_rate=learning_rate)
        posterior = inference.build_posterior(density_estimator, rejection_sampling_parameters={'enable_transform': False})
    ...
    for i in range(num_rounds):
        ...
        np.savetxt(f'output/{dir_prefix}snpec_post_round{i+1}_seed{seed}.csv',
...
if __name__ == "__main__":
    ...
    for seed in config.seeds:
        two_moons_snpe_c(num_simulations_per_round=config.num_simulations_per_round,
