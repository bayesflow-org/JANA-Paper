...
def two_moons_snpla(num_rounds,
                    seed,
                    prior,
                    simulator,
                    dir_prefix=""):
    ...
    optimizer_post = torch.optim.Adam(flow_post.parameters(), lr=0.001, weight_decay=0.0)
    decay_rate_post = 0.9
    ...
    models_lik, models_post = inference_snpla(flow_lik,
                                              flow_post,
                                              prior,
                                              simulator,
                                              optimizer_lik,
    ...
    for i in range(num_rounds):
        ...
        np.savetxt(f'output/{dir_prefix}snpla_post_round{i + 1}_seed{seed}.csv',
    ...
    for i in range(num_rounds):
        ...
        np.savetxt(f'output/{dir_prefix}snpla_post_pred_round{i + 1}_seed{seed}.csv',
...
if __name__ == "__main__":
    ...
    for seed in config.seeds:
        two_moons_snpla(num_rounds=config.num_rounds,
                        seed=seed,
