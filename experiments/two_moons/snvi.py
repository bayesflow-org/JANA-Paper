...
import sbi
from sbi.inference import SNLE_A, ImportanceSamplingPosterior, likelihood_estimator_based_potential
...
def two_moons_snvi(num_simulations_per_round,
                   num_rounds,
                   seed,
                   prior,
                   simulator,
    ...
    for i in range(num_rounds):
        ...
        likelihood_estimator = inference.append_simulations(theta, x).train(
            learning_rate=learning_rate, training_batch_size=1000,
        )
        potential, transform = likelihood_estimator_based_potential(likelihood_estimator, prior, x_o)

        if i == 0:
            vi_posterior = inference.build_posterior(sample_with="vi", vi_method="fKL", vi_parameters={"q": "nsf"})
        else:
            vi_posterior = inference.build_posterior(sample_with="vi", vi_method="fKL",
                                                     vi_parameters={"q": vi_posterior})

        vi_posterior = vi_posterior.set_default_x(x_o)
        vi_posterior.train(learning_rate=1e-3, gamma=0.9999, max_num_iters=1000, min_num_iters=100, )
        proposal = ImportanceSamplingPosterior(potential, vi_posterior, transform, oversampling_factor=8)
    ...
    for i in range(num_rounds):
        ...
        np.savetxt(f'output/{dir_prefix}snvi_post_round{i+1}_seed{seed}.csv',
    ...
    for i in range(num_rounds):
        ...
        np.savetxt(f'output/{dir_prefix}snvi_post_pred_round{i+1}_seed{seed}.csv',
    ...
    with open(f'output/{dir_prefix}snvi_metadata_seed{seed}.txt', 'w') as f:
        ...
        f.write(f'Inference time (posterior predictive) avg per round: {run_time_posterior_predictive_inference:.4f}\n')
...
if __name__ == "__main__":
    ...
    for seed in config.seeds:
        two_moons_snvi(num_simulations_per_round=config.num_simulations_per_round,
