...
coupling_settings = {
    "t_args": {
        "dense_args": dict(
            units=32, kernel_initializer="glorot_uniform", activation="tanh"
        ),
        "num_dense": 2,
    },
    "s_args": {
...
likelihood_net = InvertibleNetwork(
    num_params=2, num_coupling_layers=12, coupling_settings=coupling_settings
)
...
for i in range(0, len(file_list)):
    ...
    add_data = feather_loader(
        presimulation_path + "/" + file_list[i], param_names=param_names
    )
    if training_data is None:
        training_data = add_data
    else:
        training_data["prior_draws"] = np.concatenate(
            (training_data["prior_draws"], add_data["prior_draws"])
        )
        training_data["sim_data"] = np.concatenate(
            (training_data["sim_data"], add_data["sim_data"])
        )
