...
stan_model = CmdStanModel(stan_file=stan_file, cpp_options={"STAN_THREADS": False})
...
def load_data(N, file_path):
    ...
    sim_data = np.reshape(
        data[(first * num_trials) : (last * num_trials)], (N, num_trials, num_data_cols)
    )
    return sim_data
...
for i in range(N):
    ...
    samples_stan[i] = np.stack(
        [variables["v"], variables["a"], variables["t0"], variables["w"]], axis=-1
    )
